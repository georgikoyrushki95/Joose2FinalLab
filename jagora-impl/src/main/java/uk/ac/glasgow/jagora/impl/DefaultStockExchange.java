...
public class DefaultStockExchange implements StockExchange {
	...
	public void doClearing() {
		...
		//right after the clearing is done, we would like to notify the observing traders (ones that have subscribed)
		notifyObservers();
	...
	public void notifyObservers() {
		//returns an unmodifiable view of the tradeHistory which will be passed to the Observers 
		//we don't want any of them to be able to change anything inside the list
		List<TickEvent<Trade>> unmodifiableTradeHistory = Collections.unmodifiableList(tradeHistory);
		for (Trader subscr: subscribers){
			subscr.update(unmodifiableTradeHistory);
		}
	}
